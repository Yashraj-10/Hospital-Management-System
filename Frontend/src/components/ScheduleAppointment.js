...
const ScheduleAppointment = () => {
    ...
    const handleSelectDoctor = (e) => {
        ...
        // )
            ...
            axios.post('/doc_appointment/dates?doc_id='.concat(`${docSelect}`), {
                access_token: localStorage.getItem('access_token')
            })
                .then((response) => {
                    console.log(response.data);
    ...
    const handleDateClick = (e) => {
        ...
        if (appointmentDate.length == 0) {
            ...
            axios
                .post(`/doc_appointment/slots?doc_id=${docSelect}&date=${final}`, {
                    access_token: localStorage.getItem("access_token")
                })
                .then(
                    (response) => {
                                    ...
                                    >
                                        ...
                                        {doctorsAvailable.map((doctor) => (
                                            <option key={doctor.user_id} value={doctor.user_id}>
                                                {doctor.name} - {doctor.docType}({doctor.user_id})
                                            </option>
                                        ))}
                                    </select>
                                </div>
