...
def logout_user():
    ...
    val, current_user_id = check_token(access_token, ['fdo', 'doc', 'deo', 'dba'])
    if val == 401:
    ...
    ########################################
    # we will surely get a user corresponding to the access token
    # we either come here or we get user session expired

    conn = get_db_connection()
    cur = conn.cursor()

    cur.execute("UPDATE users SET access_token = '' WHERE access_token = '"+access_token+"';")
    conn.commit()
    conn.close()

    return jsonify(message="User logged out", user_id=current_user_id), 200
    # direct to login page after this
